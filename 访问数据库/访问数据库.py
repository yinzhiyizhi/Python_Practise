...
# 程序运行的时候，数据都是在内存中的。
# 当程序终止的时候，通常都需要将数据保存到磁盘上，
# 无论是保存到本地磁盘，还是通过网络保存到服务器上，
# 最终都会将数据写入磁盘文件。
...
# 存储和读取需要自己实现，JSON还是标准，
# 自己定义的格式就各式各样了；

# 不能做快速查询，只有把数据全部读到内存中才能自己遍历，
# 但有时候数据的大小远远超过了内存（比如蓝光电影，40GB的数据），
# 根本无法全部读入内存。

# 为了便于程序保存和读取数据，
# 而且，能直接通过条件快速查询到指定的数据，
...
# 根据某个年级的ID就可以查找所有班级的行，
# 这种查询语句在关系数据库中称为SQL语句，可以写成：
...
# 由于本教程不涉及到关系数据库的详细内容，
# 如果你想从零学习关系数据库和基本的SQL语句，如
# 果你想从零学习关系数据库和基本的SQL语句，请自行搜索相关课程。
...
# 这些数据库都是不开源而且付费的，
# 最大的好处是花了钱出了问题可以找厂家解决，
# 不过在Web的世界里，常常需要部署成千上万的数据库服务器，
# 当然不能把大把大把的银子扔给厂家，
# 所以，无论是Google、Facebook，还是国内的BAT，
# 无一例外都选择了免费的开源数据库：
...
# 作为Python开发工程师，选择哪个免费数据库呢？
# 当然是MySQL。
# 因为MySQL普及率最高，出了错，可以很容易找到解决方法。
# 而且，围绕MySQL有一大堆监控和运维的工具，安装和使用很方便。

# 为了能继续后面的学习，
# 你需要从MySQL官方网站下载并安装MySQL Community Server 5.6
