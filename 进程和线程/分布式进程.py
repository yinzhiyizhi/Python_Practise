...
# 我们先看服务进程，服务进程负责启动Queue，把Queue注册到网络上，
# 然后往Queue里面写入任务：
...
# 绑定端口5000，设置验证码‘abc’：
manager=QueueManager(addresss=('',5000),authkey=b'abc')
# 启动Queue：
manager.start()
# 获得通过网络访问的Queue对象：
...
# 由于这个QueueManager只从网络上获取Queue，所以在注册时只提供名字：
...
# 所以，Queue对象存储在task_master.py进程中：
